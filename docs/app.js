...
async function fetchOpenApiJson(url) {
  ...
  if (!trimmed) throw new Error("Enter a Swagger/OpenAPI URL or upload a JSON file.");
...
els.form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (swaggerF && swaggerF.length) {
      ...
      try {
        openapi = await fetchOpenApiJson(els.swaggerUrl.value);
      } catch (fetchErr) {
        const msg =
          fetchErr.message ||
          "Could not load OpenAPI from URL. Browsers block many APIs (CORS). Upload openapi.json / swagger.json as a file, or use a URL that sends Access-Control-Allow-Origin.";
        showError(msg);
        return;
      }
    }
